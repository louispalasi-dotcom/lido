...
// Crée une opportunité reliée à un compte existant, pré-remplie depuis le compte.
export default function OpportunityDrawer({
  ...
  useEffect(() => {
    if (!account) return;
    setTitle(`${clientDisplayName(account)} — opportunité`);
    setAmount("");
    setQuoteValue("");
    setProbability("50");
  ...
  async function creer() {
    ...
    try {
      await createOpportunity({
        ...
        title: title.trim() || clientDisplayName(account),
      ...
      setError("La création a échoué. Es-tu bien connecté ?");
    } finally {
      setSaving(false);
    }
  }
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between border-b border-[#E6EAF0] px-5 py-4">
          <div>
            <h2 className="text-base font-semibold text-[#0A2540]">Nouvelle opportunité</h2>
            {account && (
              <p className="text-xs text-[#94A3B8]">Compte : {clientDisplayName(account)}</p>
            )}
          </div>
          <button
            onClick={onClose}
